...
class Queue:
    ...
    async def get_task(self, task: Task | UUID | list[Task | UUID]) -> dict[UUID, Task] | Task | None:
        """Get task by id.
    ...
    @overload
    async def wait_for(self, task: Task, timeout: float = 0) -> Task | None: ...
    @overload
    async def wait_for(self, task: UUID | str, timeout: float = 0) -> Task | None: ...
    @overload
    async def wait_for(self, task: list[Task], timeout: float = 0) -> dict[UUID, Task]: ...
    @overload
    async def wait_for(self, task: list[UUID], timeout: float = 0) -> dict[UUID, Task]: ...

    async def wait_for(self, task: Task | UUID | list[Task | UUID], timeout: float = 0) -> dict[UUID, Task] | Task | None:
